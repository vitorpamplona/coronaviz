    ...
    if (newArr.locations) {
        ...
        for (var i=0; i<newArr.locations.length; i++) {
            ...
            if (newArr.locations[i].timestampMs > STARTING_DATE) {
                locations.push( new LatLngTime( 
                    newArr.locations[i].latitudeE7 * (10 ** -7), 
                    newArr.locations[i].longitudeE7 * (10 ** -7),
                    parseInt(newArr.locations[i].timestampMs)));
            }
        }    
    }
        ...
        for (var i=0; i<newArr.timelineObjects.length; i++) {
            ...
            if (newArr.timelineObjects[i].placeVisit && newArr.timelineObjects[i].placeVisit.location) {
                if (newArr.timelineObjects[i].placeVisit.duration.startTimestampMs > STARTING_DATE) {
                    locations.push(new LatLngTime( 
                       newArr.timelineObjects[i].placeVisit.location.latitudeE7 * (10 ** -7), 
                       newArr.timelineObjects[i].placeVisit.location.longitudeE7 * (10 ** -7), 
                       parseInt(newArr.timelineObjects[i].placeVisit.duration.startTimestampMs)));
                }
            } 
            ...
            if (newArr.timelineObjects[i].activitySegment && newArr.timelineObjects[i].activitySegment.startLocation) {
                if (newArr.timelineObjects[i].activitySegment.duration.startTimestampMs > STARTING_DATE) {
                    locations.push(new LatLngTime( 
                         newArr.timelineObjects[i].activitySegment.startLocation.latitudeE7 * (10 ** -7), 
                         newArr.timelineObjects[i].activitySegment.startLocation.longitudeE7 * (10 ** -7), 
                         parseInt(newArr.timelineObjects[i].activitySegment.duration.startTimestampMs)));
                }
            } 
        }
    }
